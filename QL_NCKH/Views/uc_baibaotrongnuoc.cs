...
namespace QL_NCKH
{
    public partial class uc_baibaotrongnuoc : DevExpress.XtraEditors.XtraUserControl
    {
        MyClass my = new MyClass();
        private List<string> productList;
        const string LoaiBaiBao = "Trong nước";
        private string mabb;
        public uc_baibaotrongnuoc()
        {
            InitializeComponent();
        }
        public string Mabb
        ...
        public void LoadDL()
        {
            try
            {
                ...
                dgv_baibao.Columns[8].Width = 200;
        ...
        private void uc_baibaotrongnuoc_Load(object sender, EventArgs e)
        {
            LoadDL();
        ...
        {
            int nam;
            if (!int.TryParse(txt_namxb.Text, out nam))
            {
                return false;
            }
                ...
                if (kiemtra())
                {
                    ...
                    if (kiemtraSo())
                    {
                        if (!kiemTraMa(txt_ma.Text))
                        {
                            ...
                            {
                                string query = "delete from TacGiaBaiBao where MaBB=@MaBB";
                                SqlCommand commandtg = my.SqlCommand(query);
                                commandtg.Parameters.AddWithValue("@MaBB", Mabb);
        ...
        public void ExcelExport()
        {
            try
            {
                ...
                head.Value2 = "DANH SÁCH BÀI BÁO TRONG NƯỚC";
                ...
                if (cbo_loai.SelectedIndex != -1)
                {
                    ...
                    switch (i)
                    {
                        ...
                        case 1:
                            try
                            {
                                sql = "select MaBB,TenBaiBao,TenTapChi,SoVaThoiGianXB,ChiSoISN,NamXB,DiemCongTrinh,GhiChu,TacGia from BaiBao where LoaiBaiBao =N'" + LoaiBaiBao + "' and TenBaiBao like N'%"+txt_timkiem.Text+"%' ";
