...
namespace QL_NCKH
{
    public partial class uc_cuocthiSTKNcaptruong : DevExpress.XtraEditors.XtraUserControl
    {

        MyClass my = new MyClass();
        private List<string> productListGV;
        private List<string> productListSV;
        private List<string> productListSVNT;
        private string mact;

        public uc_cuocthiSTKNcaptruong()
        {
            InitializeComponent();
        }
        public string Mact
        ...
        public void loadDL()
        {
            try
            {
                ...
                string query = " select MaCuocThi,TenCuocThi,LinhVuc,NamTC,KinhPhi,LoaiCuocThi from CuocThiSTKN WHERE CapCT = N'Cấp Trường' ";
        ...
        public void loadDLTVDTNT(string ma)
        {
            try
            {

                string query = @" select ThanhVienCuocThiNT.MaSVNT,SinhVienNgoaiTruong.HoTen,ThanhVienCuocThiNT.ChucVu 
                                from ThanhVienCuocThiNT,SinhVienNgoaiTruong 
                                where ThanhVienCuocThiNT.MaSVNT = SinhVienNgoaiTruong.MaSVNT and ThanhVienCuocThiNT.MaDoi =  '" + ma + "' ";
                DataTable dt = my.DocDL(query);
                dgv_svnt.DataSource = dt;
                dgv_svnt.Columns[0].HeaderText = "Mã sinh viên";
                dgv_svnt.Columns[1].HeaderText = "Tên sinh viên";
                dgv_svnt.Columns[2].HeaderText = "Chức vụ";
        ...
        private void uc_cuocthiSTKNcaptruong_Load(object sender, EventArgs e)
        {
            try
            {
                loadDL();
        ...
        private void dgv_sv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show("$ Lỗi hiển thị thông tin sinh viên", "Lỗi");
            }
        }
        ...
        public bool Ktra()
        {
            if (string.IsNullOrWhiteSpace(txt_mact.Text) || string.IsNullOrWhiteSpace(txt_linhvuc.Text)
                ...
                || string.IsNullOrWhiteSpace(txt_nam.Text) || string.IsNullOrWhiteSpace(cbo_loaict.Text) )
                return false;
                                    ...
                                    if (maDT.Count > 0)
                                    {
                                        ...
                                        for (int i = 0; i < maDT.Count; i++)
                                        {
                                            ...
                                            if (xoaGV >= 0)
                                            {
                                                ...
                                                if (xoaSV >= 0)
                                                {
                                                    string querySVNT = "delete from ThanhVienCuocThiNT where MaDoi = @Madoi";
                                                    SqlCommand commandSVNT = my.SqlCommand(querySVNT);
                                                    commandSVNT.Parameters.AddWithValue("@Madoi", ma);
                                                    int xoaSVNT = commandSVNT.ExecuteNonQuery();
                                                    if (xoaSVNT >= 0)
                                                    {
            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("Thành viên đã có trong đội thi !", "Thông báo");
                        }
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (cbo_tk.SelectedIndex == 0)
                    {
                        try
                        {
                            string query = " select MaCuocThi, TenCuocThi, LinhVuc, NamTC, KinhPhi, LoaiCuocThi from CuocThiSTKN WHERE CapCT = N'Cấp Trường'  and MaCuocThi like '%" + txt_timkiem.Text + "%' ";
                            DataTable dt = my.DocDL(query);
                            dgv_ct.DataSource = dt;
                            dgv_ct.Columns[0].HeaderText = "Mã cuộc thi";
                            dgv_ct.Columns[1].HeaderText = "Tên cuộc thi";
                    ...
                    else if (cbo_tk.SelectedIndex == 1)
                    {
                        try
                        {
                            ...
                            string query = " select MaCuocThi, TenCuocThi, LinhVuc, NamTC, KinhPhi, LoaiCuocThi from CuocThiSTKN where CapCT = N'Cấp Trường'  and TenCuocThi like N'%" + txt_timkiem.Text + "%' ";
                            DataTable dt = my.DocDL(query);
                            dgv_ct.DataSource = dt;
                            dgv_ct.Columns[0].HeaderText = "Mã cuộc thi";
                            dgv_ct.Columns[1].HeaderText = "Tên cuộc thi";
        ...
        public void ExcelExportDSCT()
        {
            try
            {
                ...
                head.Value2 = "DANH SÁCH CUỘC THI CẤP TRƯỜNG";
        ...
        public void excelCT()
        {
            try
            {
                ...
                head.Value2 = "DANH SÁCH CHI TIẾT CÁC CUỘC THI CẤP TRƯỜNG  ";
                ...
                cl7.Value = "Sinh viên tham gia - Chức vụ";
                ...
                for (int i = 0; i < dataTable.Rows.Count; i++)
                {
                    ...
                    //
                    string query = "select MaDoi,TenDoi,TenYTuong,DonVi from DoiThamGiaCuocThi where MaCuocThi = '" + maCT + "' ";
                    ...
                    for (int row = 0; row < dt.Rows.Count; row++)
                    {
                        ...
                        string cel = dt.Rows[row]["MaDoi"].ToString() + "-" + dt.Rows[row]["TenDoi"].ToString() +"-"+ dt.Rows[row]["TenYTuong"].ToString() +"-"+ dt.Rows[row]["DonVi"].ToString() + "\n";
                        line1.Value += cel;
                        //
                        ...
                        //
                        //
                        string svnt = @"SELECT ThanhVienCuocThiNT.MaDoi,SinhVienNgoaiTruong.HoTen, ThanhVienCuocThiNT.ChucVu 
                                        FROM ThanhVienCuocThiNT,SinhVienNgoaiTruong 
                                        WHERE ThanhVienCuocThiNT.MaSVNT = SinhVienNgoaiTruong.MaSVNT AND ThanhVienCuocThiNT.MaDoi = '" + maDT + "' ";

                        DataTable tvnt = my.DocDL(svnt);



                        for (int r1 = 0; r1 < tvnt.Rows.Count; r1++)
                        {

                            string celSVNT = tvnt.Rows[r1]["MaDoi"].ToString() + "-" + tvnt.Rows[r1]["HoTen"].ToString() + "-" + tvnt.Rows[r1]["ChucVu"].ToString() + "\n";
                            line3.Value += celSVNT;
        ...
        public DataTable LayDuLieuBaoCao1CT()
        {
            ...
            string query = " select MaCuocThi,TenCuocThi,LinhVuc,NamTC,KinhPhi,LoaiCuocThi from CuocThiSTKN where CapCT = N'Cấp Trường' and MaCuocThi ='" + txt_mact.Text + "' ";
            DataTable dataTable = my.DocDL(query);
        ...
        public void excelCT1CT()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Vui lòng chọn cuộc thi cần export dữ liệu","Thông báo");
                }
            }
        ...
        private void LoadProductListSVNT()
        {
            try
            {

                productListSVNT = new List<string>();
                string query = "SELECT MaSVNT FROM SinhVienNgoaiTruong";
                DataTable tb = my.DocDL(query);
                if (tb.Rows.Count > 0)
                {
                    for (int i = 0; i < tb.Rows.Count; i++)
                    {
                        string ma = tb.Rows[i][0].ToString();
                        productListSVNT.Add(ma);
                    }
                }
            }
            catch
            {
                MessageBox.Show($"Lỗi thực hiện tạo danh sách sinh viên ngoài trường", "Lỗi");
            }

        }
        private void ShowSuggestionsSVNT(List<string> suggestions)
        {
            list_svnt.Items.Clear();
            list_svnt.Items.AddRange(suggestions.ToArray());

            list_svnt.Visible = suggestions.Any();
        }
        private void txt_masvnt_TextChanged(object sender, EventArgs e)
        {
            string searchTerm = txt_masvnt.Text.ToLower();
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                List<string> filteredProducts = productListSVNT
               .Where(product => product.ToLower().Contains(searchTerm))
               .ToList();

                if (filteredProducts != null)
                {
                    ShowSuggestionsSVNT(filteredProducts);
                }
                else
                {
                    list_svnt.Visible = false;
        ...
        private void list_svnt_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (list_svnt.SelectedItem != null)
            {
                string selectedProduct = list_svnt.SelectedItem.ToString();
                if (!string.IsNullOrWhiteSpace(selectedProduct))
                {
                    txt_masvnt.Text = selectedProduct;
                    list_svnt.Visible = false;
                    string sql = "select HoTen from SinhVienNgoaiTruong where MaSVNT = '" + selectedProduct + "' ";
                    DataTable tb = my.DocDL(sql);
                    if (tb.Rows.Count > 0)
                    {
                        string hoten = tb.Rows[0][0].ToString();
                        txt_tensvnt.Text = hoten;
                    }
        ...
        public bool KtraMaTVDTNT(string madt, string masv)
        {
            try
            {
                string sql = "select * from ThanhVienCuocThiNT where MaDoi = '" + madt + "' and MaSVNT = '" + masv + "' ";
                DataTable tb = my.DocDL(sql);
                if (tb.Rows.Count > 0)
                {
                    return false;
            ...
            {
                MessageBox.Show("Lỗi kiểm tra mã thành viên ngoài trường !", "Thông báo");
            }
            return true;
        }
        private void btn_joinsvnt_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txt_masvnt.Text) || string.IsNullOrWhiteSpace(txt_tensvnt.Text) || string.IsNullOrWhiteSpace(cbo_chucvunt.Text))
            {
                MessageBox.Show("Vui lòng nhập đầy đủ thông tin !", "Thông báo");
            }
            else
            {
                try
                {
                    ...
                    {
                        string mact = txt_madoithi.Text;
                        if (KtraMaTVDTNT(txt_madoithi.Text, txt_masvnt.Text))
                        {
                            string sql = "insert into ThanhVienCuocThiNT values (@Masv,@Madoi,@ChucVu)";
                            SqlCommand comd = my.SqlCommand(sql);
                            comd.Parameters.AddWithValue("@Masv", txt_masvnt.Text);
                            comd.Parameters.AddWithValue("@Madoi", txt_madoithi.Text);
                            comd.Parameters.AddWithValue("@ChucVu", cbo_chucvunt.Text);
        ...
        private void btn_xoasvnt_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txt_masvnt.Text) || string.IsNullOrWhiteSpace(txt_tensvnt.Text) || string.IsNullOrWhiteSpace(cbo_chucvunt.Text))
            {
                MessageBox.Show("Vui lòng nhập đầy đủ thông tin !", "Thông báo");
            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!KtraMaTVDTNT(txt_madoithi.Text, txt_masvnt.Text))
                        {
                            string sql = "delete from ThanhVienCuocThiNT where MaSVNT=@Masv AND MaDoi=@Madoi";
                            SqlCommand comd = my.SqlCommand(sql);
                            comd.Parameters.AddWithValue("@Masv", txt_masvnt.Text);
                            comd.Parameters.AddWithValue("@Madoi", txt_madoithi.Text);
        ...
        private void btn_suasvnt_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txt_masvnt.Text) || string.IsNullOrWhiteSpace(txt_tensvnt.Text) || string.IsNullOrWhiteSpace(cbo_chucvunt.Text))
            {
                MessageBox.Show("Vui lòng nhập đầy đủ thông tin !", "Thông báo");
            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!KtraMaTVDTNT(txt_madoithi.Text, txt_masvnt.Text))
                        {
                            string sql = "update ThanhVienCuocThiNT set ChucVu=@Chucvu where MaSVNT=@Masv AND MaDoi=@Madoi";
                            SqlCommand comd = my.SqlCommand(sql);
                            comd.Parameters.AddWithValue("@Masv", txt_masvnt.Text);
                            comd.Parameters.AddWithValue("@Madoi", txt_madoithi.Text);
                            comd.Parameters.AddWithValue("@Chucvu", cbo_chucvunt.Text);
        ...
        private void dgv_svnt_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                txt_masvnt.Text = dgv_svnt.CurrentRow.Cells[0].Value.ToString();
                txt_tensvnt.Text = dgv_svnt.CurrentRow.Cells[1].Value.ToString();
                cbo_chucvunt.Text = dgv_svnt.CurrentRow.Cells[2].Value.ToString();
            }
            catch
            {
                MessageBox.Show("$ Lỗi hiển thị thông tin sinh viên ngoài trường", "Lỗi");
            }
        }

        private void barButtonItem10_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            frm_bomaytochuc frm = new frm_bomaytochuc();
            frm.Mact = txt_mact.Text;
            frm.ShowDialog();
        }
        ...
        private void button_tk_Click_1(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(cbo_tk.Text))
            {
                MessageBox.Show("Vui nhập chọn khóa tìm kiếm  !", "Thông báo");
