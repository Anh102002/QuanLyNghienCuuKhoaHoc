...
namespace QL_NCKH
{
    public partial class uc_sinhvien : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void LoadDL()
        {
            try
            {
                ...
                if(tb.Rows.Count > 0)
                {
                    ...
                    dgv_sinhvien.Columns[3].HeaderText = "Lớp";
                    dgv_sinhvien.Columns[4].HeaderText = "Khoa";
                    dgv_sinhvien.Columns[5].HeaderText = "Email";
                    dgv_sinhvien.Columns[5].Width = 150;
                    dgv_sinhvien.Columns[6].HeaderText = "Số điện thoại";
                    dgv_sinhvien.Columns[7].HeaderText = "Cơ sở";
                    dgv_sinhvien.Columns[8].HeaderText = "Giới tính";
        ...
        public bool kiemtra()
        {
            if (string.IsNullOrWhiteSpace(txt_masv.Text) || string.IsNullOrWhiteSpace(txt_hoten.Text) 
                || string.IsNullOrWhiteSpace(txt_lop.Text) || string.IsNullOrWhiteSpace(cb_khoa.Text)
                || string.IsNullOrWhiteSpace(txt_email.Text) || string.IsNullOrWhiteSpace(cb_coso.Text) 
                    ...
                    if (kiemTraMa(txt_masv.Text))
                    {
                        if (IsValidEmail(txt_email.Text))
                        {
                            if (kiemtraSoDT(txt_sodt.Text))
                            {
                                ...
                                string sql = "insert into SinhVien values('" + txt_masv.Text + "',N'" + txt_hoten.Text + "','" + ngaysinh + "','" + txt_lop.Text + "',N'" + cb_khoa.Text + "','" + txt_email.Text + "','" + txt_sodt.Text + "',N'" + cb_coso.Text + "',N'" + cb_gioitinh.Text + "')";
                    ...
                    if (kiemtra())
                    {
                        ...
                        if (!kiemTraMa(txt_masv.Text))
                        {
                            if (IsValidEmail(txt_email.Text))
                            {
                                ...
                                {
                                    string ngaysinh = dtp_ngaysinh.Value.ToString("yyyy/MM/dd");
                                    string sql = "update SinhVien set HoTen = N'" + txt_hoten.Text + "',NgaySinh='" + ngaysinh + "',Lop='" + txt_lop.Text + "',Khoa=N'" + cb_khoa.Text + "',Email='" + txt_email.Text + "',SoDT='" + txt_sodt.Text + "',CoSo=N'" + cb_coso.Text + "',GioiTinh=N'" + cb_gioitinh.Text + "' where  MaSV='" + txt_masv.Text + "' ";
                                    int  up =my.Update(sql);
                                    if(up>0)
                                    {
                ...
                if (!string.IsNullOrWhiteSpace(txt_masv.Text) )
                {
                    ...
                    if (!kiemTraMa(txt_masv.Text))
                    {
                        if (IsValidEmail(txt_email.Text))
                        {
                            if (kiemtraSoDT(txt_sodt.Text))
                            {
                                bool XOATVCT = xoa.XoaTVCT(txt_masv.Text);
                                if (XOATVCT)
                                {
                                    bool XOASVDT = xoa.XoaSVDT(txt_masv.Text);
                                    if(XOASVDT)
                                    {
                                        ...
                                        {
                                            bool XOAGTSV = xoa.XoaGTSV(txt_masv.Text);


                                            if (XOAGTSV)
                                            {
                                                bool XOASV = xoa.XoaSinhVien(txt_masv.Text);
                                                if (XOASV)
                                                {
        ...
        private void dgv_sinhvien_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txt_lop.Text = dgv_sinhvien.CurrentRow.Cells[3].Value.ToString();
            cb_khoa.Text = dgv_sinhvien.CurrentRow.Cells[4].Value.ToString();
            txt_email.Text = dgv_sinhvien.CurrentRow.Cells[5].Value.ToString();
            txt_sodt.Text = dgv_sinhvien.CurrentRow.Cells[6].Value.ToString();
            cb_coso.Text = dgv_sinhvien.CurrentRow.Cells[7].Value.ToString();
            cb_gioitinh.Text = dgv_sinhvien.CurrentRow.Cells[8].Value.ToString();
        }
                ...
                else
                {
                    if (cbo_tk.Text == "Mã sinh viên")
                    {
                        try
                        {
                            ...
                            string sql = "select * from SinhVien where MaSV like '%" + txt_timkiem.Text + "%' ";
                            DataTable tb = my.DocDL(sql);
                            dgv_sinhvien.DataSource = tb;
                            dgv_sinhvien.Columns[0].HeaderText = "Mã sinh viên";
                            dgv_sinhvien.Columns[1].HeaderText = "Họ tên";
                    ...
                    else if (cbo_tk.Text == "Họ tên")
                    {
                        try
                        {
                            string sql = "select * from SinhVien where HoTen like N'%" + txt_timkiem.Text + "%' ";
                            DataTable tb = my.DocDL(sql);
                            dgv_sinhvien.DataSource = tb;
                            dgv_sinhvien.Columns[0].HeaderText = "Mã sinh viên";
                            dgv_sinhvien.Columns[1].HeaderText = "Họ tên";
                    ...
                    else
                    {
                        try
                        {
                            string sql = "select * from SinhVien where Lop like N'%" + txt_timkiem.Text + "%' ";
                            DataTable tb = my.DocDL(sql);
                            dgv_sinhvien.DataSource = tb;
                            dgv_sinhvien.Columns[0].HeaderText = "Mã sinh viên";
                            dgv_sinhvien.Columns[1].HeaderText = "Họ tên";
                        ...
                        {
                            MessageBox.Show("Lỗi tìm kiếm theo lớp sinh viên  !", "Thông báo");
                        }
                    }
        ...
        private void txt_timkiem_SelectedIndexChanged(object sender, EventArgs e)
        {
